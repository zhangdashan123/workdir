...
class ExPeDiaSpider(object):
    def __init__(self, detail_url, hotel_name, area):
        self.my = MySql()
    ...
    def get_data(self):
        try:
            resp = requests.get(url=self.detail_url, headers=self.headers, verify=False)
            if resp.status_code == 200:
                regex = re.compile(r'chkin=(.*?)&chkout=(.*?)&', re.S)
                date_info = regex.findall(self.detail_url)
                ...
                if 'infosite.offersData' in str_data:
                    ...
                    for resp_data in resp1:
                        ...
                        if house_price and '房' in house_name:
                            ...
                            print(house_name, house_price, begin_date, end_date, self.hotel_name, self.area)
...
if __name__ == '__main__':
    # url = 'https://www.expedia.com/cn/Hangzhou-Hotels-Conrad-Hangzhou.h27721578.Hotel-Information?adults=2&children=0&chkin=2019/09/19&chkout=2019/09/20&hwrqCacheKey=c5af0e72-ed39-4922-b542-66aefae94230HWRQ1568771097347&cancellable=false&regionId=6084457&vip=false&=undefined&exp_dp=0&exp_ts=1568771097565&exp_curr=CNY&daysInFuture=&stayLength=&ts=1568797999361'
    ...
    area = '杭州'
    hotel_name = '香格里拉酒店'
    ep = ExPeDiaSpider(url, hotel_name, area)
    ep.get_data()
